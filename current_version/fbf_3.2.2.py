...
#                                                 +#+#+#+#+#+   +#+            #
#    Created: 2022/02/16 12:08:09 by jbuny-fe          #+#    #+#              #
#    Updated: 2022/02/16 12:08:56 by jbuny-fe         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
def internet_on():  # checks for internet connection
    try:
        urlopen('http://142.250.201.78', timeout=1)
        return True
    except:
    ...
    try:
        with open('time_keeper', 'r') as f:  # opens a time_keeper file if it already exists
            ...
            while True:
                ...
                global tad  # time active to show on discord without the seconds variable
                tad = (
...
def plant_finder():  # Creates a new plant file if one isn't found
    try:
        global line_count
        line_count = 0
...
def config_finder():
    try:
        ...
        with open("config.toml", 'a') as f:
            ...
            f.close()
            print('Your setup is now complete!')
            time.sleep(2)
            config_finder()
        ...
        if start == 'Y':
            ...
            try:
                with open('lcf', 'r') as f:
                    lcf = toml.load(f)
                    global loop_counter
                    loop_counter = lcf['loop_counter']
                    f.close()
            except:
                with open('lcf', 'w') as f:
                    f.write('loop_counter = 0')
                    f.close()
                    with open('lcf', 'r') as f:
                        lcf = toml.load(f)
                        loop_counter = lcf['loop_counter']
                            ...
                            try:
                                for plant in f.readlines():
                                    ...
                                    if payload_counter == line_count:
                                        ...
                                        try:
                                            with open('lcf', 'w') as f:
                                                f.write('loop_counter = ' + str(lcff))
                                                f.close()
                                        except:
                                            print('Unable to update loop counter file')
                                        print('\n')
                                            ...
                                            if lcm == 1:
                                                payload = {
                                                    'content': 'So far the farmer has watered your plants ' + str(
                                                        loop_counter) + ' times, and has been awake for ' + str(tad)
                                        ...
                                        def animate():
                                            ...
                                            for c in itertools.cycle(['|', '/', '-', '\\']):
                                                if done:
                                                    break
                                                sys.stdout.write(
                                                    '\rThe farmer has been awake for ' + str(
                                                        time_active) + '. Waiting for '
